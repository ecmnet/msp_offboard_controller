...
RapidTrajectoryGenerator::RapidTrajectoryGenerator()
{
  Reset();
}
...
void RapidTrajectoryGenerator::setInitialState(const Vec3 x0, const Vec3 v0, const Vec3 a0, const Vec3 gravity)
{
  //initialise each axis:
  Reset();
  for(int i=0; i<3; i++) _axis[i].SetInitialState(x0[i],v0[i],a0[i]);
